...
for op,sop in lst1 + lst2:
    ...
    print RV+' '+'operator'*(sop!='min' and sop!='max')+sop+'( const Vec<T1,s1,TO1> &v1,const Vec<T2,s2,TO2> &v2 ) {'
    ...
    print RV+' '+'operator'*(sop!='min' and sop!='max')+sop+'( const Vec<T1,s1,TO1> &v1,const T2 &v2 ) {'
    print '    return '+RV+'(v1, Vec<VecCst<T2>,s1>(v2,v1.size()));'
    print '}'
    print '/** \\relates Vec */'
    print 'template<class T1,int s1,class TO1,class T2>'
    RV = 'Vec<VecOp< '+op+', 2, Vec<VecCst<T2>,s1>,false,Vec<T1,s1,TO1>,true>,s1>'
    print RV+' '+'operator'*(sop!='min' and sop!='max')+sop+'( const T2 &v2, const Vec<T1,s1,TO1> &v1 ) {'
