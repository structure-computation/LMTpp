...
inline typename TypePromote<Plus,T1,T2>::T dot(const MaterialPoint<T1,dim> &p1,const MaterialPoint<T2,dim> &p2) {
 ...
 */
template<unsigned dim,class T1,class T2>
inline MaterialPoint<typename TypePromote<Plus,T1,T2>::T,dim> operator*(const MaterialPoint<T1,dim> &p1,const T2 &p2) {
    return MaterialPoint<typename TypePromote<Plus,T1,T2>::T,dim>( p1.dep * p2, p1.rot * p2);
}
/**
 \relates LMT::MaterialPoint
 */
template<unsigned dim,class T2,class T1>
inline MaterialPoint<typename TypePromote<Plus,T1,T2>::T,dim> operator*(const T2 &p2,const MaterialPoint<T1,dim> &p1) {
    return MaterialPoint<typename TypePromote<Plus,T1,T2>::T,dim>( p2 * p1.dep, p2 * p1.rot );
}
