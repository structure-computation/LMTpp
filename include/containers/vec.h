...
\generic_comment apply
    ...
    = Description
        ...
        Ainsi si vous souhaitez faire un calcul sur les éléments d'un vecteur, vous créerez un opérateur correspondant à l'algorithme avec éventuellement des parmètres si l'algorithme a des variantes. Cela fonctionne suivant le même principe que les filtres sur une image et comme les filtres, on peut aussi enchaîner simplement les traitements ; cad par exemple  si vous avez défini des opérateurs op1, op2, ... on peut créer un autre opérateur op utilisant les op1, op2 et l'appliquer sur un vecteur via apply.
