...
/*!
    ...
    Voici un exemple où on calcule la transformée de Fourier d'un vecteur puis la transformée de Fourier inverse.
remarquez qu'on ne revient pas au vecteur initial. Il faut diviser par la longueur du vecteur pour s'y ramener.
    \Code C/C++
        #include <complex>
...
public:
    ...
    /*!
        ...
        WARNING : les performances sont excellentes lorsque n1 et n2 sont des puissances de 2 (cf algorithme de la
transformée de Fourier rapide FFT)
                  sinon elles sont moins bonnes en général, voire très mauvaises si n1 ou n2 n'est pas un multiple de
petits nombres premiers.
    */
    ...
    ImgInterp<std::complex<double>,dim,Kernel_,PT_> fft(const ImgInterp<std::complex<double>,dim,Kernel_,PT_> &i) {
        ...
        if ( init==false ) {
            if ( dim == 2 ){
                p = fftw_plan_dft_2d( i.sizes[1], i.sizes[0], const_cast<fftw_complex *>(in), out, FFTW_BACKWARD,
FFTW_ESTIMATE);
            }
            else{
                p = fftw_plan_dft_3d( i.sizes[2], i.sizes[1], i.sizes[0], const_cast<fftw_complex *>(in), out,
...
/*!
    ...
    Si div_2 est false, la transformée de Fourier , TF, est calculée sur toute l'image BBBB : i.e. sur une image 4 fois
plus grande que l'image initiale.
    Sinon une moyenne "mobile" étendue sur 4 pixels en carré est faite sur l'image BBBB avant de faire la TF. Du coup Le
résultat est une image de même taille que l'image initiale. 
...
/*!
    Cette classe sert à faire un padding qui met à zéro les pixels des bors de l'image avant de faire la transformée de
Fourier , TF.
...
//ImgInterp<std::complex<double>,dim_,Kernel_,PT_> fft( const ImgInterp<std::complex<T_>, dim_, Kernel_, PT_ > &img,
//Vec<int,dim_> topleft, Vec<int,dim_> bottomright, const TPadding padding) {
//    fft( img, topleft[0], bottomright[0], topleft[1], bottomright[1], padding);
//}
    
//template<class T_,unsigned dim_,class Kernel_,class PT_,class TPadding>
//ImgInterp<std::complex<double>,dim_,Kernel_,PT_> fft( const ImgInterp<std::complex<T_>, dim_, Kernel_, PT_ > &img, int
//xmin, int xmax, int ymin, int ymax, const TPadding padding) {
//    assert(0);
//    return ImgInterp<std::complex<double>,dim_,Kernel_,PT_>();
//}
  
//template<class T_, unsigned dim_, class Kernel_, class PT_, class TPadding>
//        ImgInterp<std::complex<double>,dim_,Kernel_,PT_> ffti( const ImgInterp<std::complex<T_>, dim_, Kernel_, PT_ >
//&img, Vec<int,2> topleft, Vec<int,2> bottomright, const TPadding padding) {
//    ffti( img, topleft[0], bottomright[0], topleft[1], bottomright[1], padding);
//}
    
//template<class T_,unsigned dim_,class Kernel_,class PT_,class TPadding>
//        ImgInterp<std::complex<double>,dim_,Kernel_,PT_> ffti( const ImgInterp<std::complex<T_>, dim_, Kernel_, PT_ >
//&img, int xmin, int xmax, int ymin, int ymax, const TPadding padding) {
//    assert(0);
//    return ImgInterp<std::complex<double>,dim_,Kernel_,PT_>();
//}
  
template<unsigned int dim_,class T_,class Kernel_,class PT_> 
ImgInterp<std::complex<double>,dim_,Kernel_,PT_> fft( const ImgInterp<std::complex<T_>, dim_, Kernel_, PT_ > &img,
Vec<int,dim_> xmin, Vec<int,dim_> xmax, const SymmetricPadding padding ) {
...
ImgInterp<std::complex<double>,dim_,Kernel_,PT_> ffti( const ImgInterp<std::complex<T_>, dim_, Kernel_, PT_ > &img,
const SymmetricPadding padding ) {
...
//template<class T_,unsigned dim_,class Kernel_,class PT_>
//        ImgInterp<std::complex<double>,dim_,Kernel_,PT_> fft( const ImgInterp<std::complex<T_>, dim_, Kernel_, PT_ >
//&img, Vec<int,2> topleft, Vec<int,2> bottomright, SymmetricPadding padding) {
//    fft( img, topleft[0], bottomright[0], topleft[1], bottomright[1], padding);
//}

template<unsigned int dim_,class T_,class Kernel_,class PT_>
ImgInterp<std::complex<double>,dim_,Kernel_,PT_> fft( const ImgInterp<std::complex<T_>, dim_, Kernel_, PT_ > &img,
Vec<int,dim_> xmin, Vec<int,dim_> xmax, ZeroPadding ) {

    Vec<int,dim_> x_in_max = xmax - xmin + 1;
    ImgInterp<std::complex<double>,dim_,Kernel_,PT_> in;

//     in.resize( x_in_max + 1 );
    in.resize( x_in_max[0] + 1, x_in_max[1] + 1 );
    in.data.set( 0 );
...
//template<class T_,unsigned dim_,class Kernel_,class PT_>
//        ImgInterp<std::complex<double>,dim_,Kernel_,PT_> fft( const ImgInterp<std::complex<T_>, dim_, Kernel_, PT_ >
//&img, Vec<int,2> topleft, Vec<int,2> bottomright, ZeroPadding padding) {
//    fft( img, topleft[0], bottomright[0], topleft[1], bottomright[1], padding);
//}
...
/*!
    Cette fonction retourne une approximation de la corrélation entre les deux images i et i2 dans le rectangle [xmin,
xmax]x[ymin, ymax].
