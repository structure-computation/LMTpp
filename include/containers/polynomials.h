...
public:
    ...
    template <int nx2, class T2, class T3> Pol (const Pol<nd,nx2,T2> &P, const Vec<int,nx> &vq, const Vec<T3,nx2-nx> &vx); ///Construit un polynome a partir d'un autre polynome avec plus de variables
    template <int nx2, class T2> Pol (const Pol<nd,nx2,T2> &P, const Vec<int,nx> &vq); ///Construit un polynome a partir d'un autre polynome avec plus de variables
...
private:
    ...
    static Vec<Vec<unsigned>,Pol<nd,nx,T>::dim> deriv_var;
    static Vec<Vec<unsigned>,Pol<nd,nx,T>::dim> deriv_ind;
    static Vec<Vec<T>,Pol<nd,nx,T>::dim> deriv_coef;
public:
    static bool init_mult;
    static void initialize_mult();
    static Vec<Vec<Vec<unsigned,2> >,Pol<nd,nx,T>::dim> mult;
...
class Pol<nd,1,T> {
public:
    static const int dim=DimPol<nd,1>::valeur;
private :
    Vec<T,dim> coefs;
public:
    static const int degree=nd;
    ...
    Pol () {}
    template <class T2> Pol (const Vec<T2> &V); ///Construit le polynome a partir de ses coefficients
    template <class T2> Pol (const Vec<T2,Pol<nd,1,T2>::dim> &V); ///Construit le polynome a partir de ses coefficients
    template <int nd2, class T2> Pol (const Pol<nd2,1,T2> &P); ///Construit un polynome a partir d'un autre polynome avec 1 variable(s)
    template <int nx2, class T2, class T3> Pol (const Pol<nd,nx2,T2> &P, const int &vq, const Vec<T3,nx2-1> &vx); ///Construit un polynome a partir d'un autre polynome avec plus de variables
    template <int nx2, class T2> Pol (const Pol<nd,nx2,T2> &P, const int &vq); ///Construit un polynome a partir d'un autre polynome avec plus de variables
    Pol (const T &a); ///Construit le polynome constant egal a a
    Pol (const T &a, const T &b, unsigned q); ///Construit le polynome a + b*Xq;
    template <class T2> Pol<nd,1,T> &operator= (const Pol<nd,1,T2> &P); ///Renvoie un polynome egal a P
    const Vec<T,dim> &coefficients() const {return coefs;} ///Renvoie les coefficients du polynome
    const Vec<Vec<unsigned,1>,Pol<nd,1,T>::dim> &powers(); ///Renvoie les puissances du polynome
    ...
    template <class T2> bool operator== (const Pol<nd,1,T2> &P) const; /// Verifie si le polynome est egal a P
    template <class T2> bool operator!= (const Pol<nd,1,T2> &P) const; /// Verifie si le polynome est different de P
    template <class T2> void operator+= (const Pol<nd,1,T2> &P); /// Ajoute P au polynome
    template <class T2> void operator-= (const Pol<nd,1,T2> &P); /// Retranche P au polynome
...
template <int nd, class T>
Vec<Vec<unsigned,1>,Pol<nd,1,T>::dim> Pol<nd,1,T>::puissances;
...
template <int nd, class T>
template <class T2>
Pol<nd,1,T>::Pol(const Vec<T2,Pol<nd,1,T2>::dim> &V) {
    for(unsigned i=0; i<min(V.size(),coefs.size()); ++i)
        coefs[i] = V[i];
    for(unsigned i=V.size(); i<coefs.size(); ++i)
        coefs[i] = 0;
...
template <int nd, class T>
template <int nx2, class T2, class T3>
Pol<nd,1,T>::Pol(const Pol<nd,nx2,T2> &P, const int &vq, const Vec<T3,nx2-1> &vx) {
...
template <int nd, class T>
template <int nx2, class T2>
Pol<nd,1,T>::Pol(const Pol<nd,nx2,T2> &P, const int &vq) {
    Vec<T,nx2-1> vx;
    vx.set(T(0));
    *this=Pol(P,vq,vx);
}
...
template <int nd, class T>
Pol<nd,1,T>::Pol(const T &a0) {
    coefs.set(T(0));
    coefs[0]=a0;
}

template <int nd, class T>
Pol<nd,1,T>::Pol(const T &a, const T &b, unsigned q) {
    coefs.set(T(0));
    coefs[0]=a;
    if (init_puissances)
        initialize_puissances();
...
template <int nd, class T>
template <class T2>
Pol<nd,1,T> &Pol<nd,1,T>::operator= (const Pol<nd,1,T2> &P) {
    coefs=P.coefficients();
    return *this;
}

template <int nd, class T>
const Vec<Vec<unsigned,1>,Pol<nd,1,T>::dim> &Pol<nd,1,T>::powers() {
    if (init_puissances)
        initialize_puissances();
    return puissances;
}
...
template <int nd, class T>
template <class T2>
bool Pol<nd,1,T>::operator== (const Pol<nd,1,T2> &P) const {
    return coefs==P.coefficients();
}
...
Pol<nd,1,typename TypePromote<Plus,T1,T2>::T> operator+ (const Pol<nd,1,T1> &P, const Pol<nd,1,T2> &Q) {
    return Pol<nd,1,typename TypePromote<Plus,T1,T2>::T>(P.coefficients()+Q.coefficients());
}

template <int nd, class T1, class T2>
Pol<nd,1,typename TypePromote<Minus,T1,T2>::T> operator- (const Pol<nd,1,T1> &P, const Pol<nd,1,T2> &Q) {
    return Pol<nd,1,typename TypePromote<Minus,T1,T2>::T>(P.coefficients()-Q.coefficients());
}
...
Pol<(nd1>nd2?nd1:nd2),1,typename TypePromote<Multiplies,T1,T2>::T> operator* (const Pol<nd1,1,T1> &P, const Pol<nd2,1,T2> &Q) {
    static const int nd = ( nd1 > nd2 ? nd1 : nd2 );
    typedef Pol<nd,1,typename TypePromote<Multiplies,T1,T2>::T> TP;
    return TP( P ) * TP( Q );
}
...
template <int nd, class T>
Pol<nd,1,T> sqrt (const Pol<nd,1,T> &P) {
    assert(0);
    /* TODO */
}
...
template <int nd1, int nd2, int nx, class T1, class T2>
Pol<(nd1>nd2?nd2:nd1),nx,typename TypePromote<Minus,T1,T2>::T> operator-(const Pol<nd1,nx,T1> &P, const Pol<nd2,nx,T2> &Q) {
    const int nd=(nd1>nd2?nd2:nd1);
...
template <int nd1, int nd2, int nx, class T1, class T2>
Pol<(nd1>nd2?nd2:nd1),nx,typename TypePromote<Multiplies,T1,T2>::T> operator*(const Pol<nd1,nx,T1> &P, const Pol<nd2,nx,T2> &Q) {
    const int nd=(nd1>nd2?nd2:nd1);
    typedef typename TypePromote<Multiplies,T1,T2>::T T;
    return converter(P,Relative_Number<nd>(),Relative_Number<nx>(),T()) * converter(Q,Relative_Number<nd>(),Relative_Number<nx>(),T());
}
...
Pol<nd,nx,typename TypePromote<Divides,T1,typename IsScalar<T2>::T>::T> operator/(const Pol<nd,nx,T1> &P, const T2 &t) {
    return Pol<nd,nx,typename TypePromote<Divides,T1,typename IsScalar<T2>::T>::T>(P.coefficients()/t);
}
