...
struct Quad_8 {
    ...
    static const char *name() { return "Quad_8"; }
    static const char *can_directly_be_represented_by() { return "Quad_8"; }
};

// --------------------------------------------------------------------------------------------------------
template<> struct NbChildrenElement<Quad_8,0> { static const unsigned res = 1; };
template<> struct NbChildrenElement<Quad_8,1> { static const unsigned res = 4; };
template<> struct NbChildrenElement<Quad_8,2> { static const unsigned res = 8; };

template<unsigned n> struct TypeChildrenElement<Quad_8,0,n> { typedef Quad_8 T; };
template<unsigned n> struct TypeChildrenElement<Quad_8,1,n> { typedef Bar_3 T; };
template<unsigned n> struct TypeChildrenElement<Quad_8,2,n> { typedef NodalElement T; };

template<class TN,class TNG,class TD,unsigned NET,class TC,class HET>
void append_skin_elements(Element<Quad_8,TN,TNG,TD,NET> &e,TC &ch,HET &het,Number<0> nvi_to_subs) {
    het.add_element(e,ch,Quad_8(),e.node(0),e.node(1),e.node(2),e.node(3),e.node(4),e.node(5),e.node(6),e.node(7));
}
...
template<class TN,class TNG,class TD,unsigned NET,class TM>
bool divide_element_using_elem_children(Element<Quad_8,TN,TNG,TD,NET> &e,TM &m,TNG **nnodes) {
...
T var_inter_insideness( const Quad_8 &e, const TV &var_inter ) {
    return min( min( min( var_inter[0], var_inter[1] ), 1 - var_inter[0] ), 1 - var_inter[1] );
}

/*!
    objectif :
        la fonction renvoie vrai si pos est dans le \a Quad_8 et faux sinon ( sous la condition que le \a Quad_8 est dans le plan ).
...
bool is_inside_linear( const Quad_8 &elem, const PosNodes &pos_nodes, const Pvec &pos ) {
