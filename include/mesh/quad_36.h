...
struct Quad_36 {
    ...
    static const unsigned nb_nodes = 36;
    static const char *name() { return "Quad_36"; }
    static const char *can_directly_be_represented_by() { return "Quad_36"; }
};

// --------------------------------------------------------------------------------------------------------
template<> struct NbChildrenElement<Quad_36,0> { static const unsigned res = 1; };
...
void append_skin_elements(Element<Quad_36,TN,TNG,TD,NET> &e,TC &ch,HET &het,Number<1> nvi_to_subs) {
    ...
    het.add_element(e,ch,Bar_6(),e.node(1),e.node(2),e.node(8),e.node(9),e.node(10),e.node(11));
    het.add_element(e,ch,Bar_6(),e.node(2),e.node(3),e.node(12),e.node(13),e.node(14),e.node(15));
    het.add_element(e,ch,Bar_6(),e.node(3),e.node(0),e.node(16),e.node(17),e.node(18),e.node(19));
}
template<class TN,class TNG,class TD,unsigned NET,class TC,class HET>
void append_skin_elements(Element<Quad_36,TN,TNG,TD,NET> &e,TC &ch,HET &het,Number<2> nvi_to_subs) {
...
void update_edge_ratio(const Element<Quad_36,TN,TNG,TD,NET> &e,TM &m,T &edge_ratio) {
...
typename TypePromote<Abs,typename TNG::T>::T measure( const Element<Quad_36,TN,TNG,TD,NET> &e ) {
...
bool divide_element(Element<Quad_36,TN,TNG,TD,NET> &e,TM &m,TNG **nnodes) {
    std::cout << "divide_element not implemented for Quad_36" << std::endl;
    assert(0);
    return false;
}

template<class TN,class TNG,class TD,unsigned NET,class TM>
bool divide_element_using_elem_children(Element<Quad_36,TN,TNG,TD,NET> &e,TM &m,TNG **nnodes) {
    return divide_element(e,m,nnodes);
}

template<class TV,class T>
bool var_inter_is_inside( const Quad_36 &, const TV &var_inter, T tol = 0 ) {
    return heaviside( var_inter[0] + tol ) * heaviside( var_inter[1] + tol ) * heaviside( 1 - var_inter[0] + tol ) * heaviside( 1 - var_inter[1] + tol );
}

/*!
    objectif :
        la fonction renvoie vrai si pos est dans le \a Quad_36 et faux sinon ( sous la condition que le \a Quad_36 est dans le plan ).
        Mais pour cet élément non linéaire , on considère le \a Quad engendré par les 4 premiers noeuds.
        
    param :
        Quad_36 : le type d'élément
...
bool is_inside_linear( const Quad_36 &elem, const PosNodes &pos_nodes, const Pvec &pos ) {
