...
public:
    Lexem();
    ~Lexem();
    void display( std::ostream &os, bool withChildren = false ) const ;
    void display_just_name_and_type( std::ostream &os ) const ;
    friend std::ostream &operator<<( std::ostream &os, Lexem &le );
...
struct Comment_zone {
    Comment_zone() { start = NULL; size = 0; }
    char* start;
    int size;
    friend std::ostream &operator<<( std::ostream &os, const Comment_zone &c ) {
        os << "[[|";
        os.write(c.start,c.size);
        os << "|]]";
        return os;
    }
};
