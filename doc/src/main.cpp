...
int main(int argc, char* argv[]) {
   ...
   // puis on définit la structure du site web.
    ...
    // on crée la liste des objets Target. Si certains objets génériques n'ont pas de commentaires \generic_comment, on crée ici leur target
    docProject.generate_ListTarget() ;

    
    docProject.generate_webpage_of_summary() ;// génère la pages web des objets génériques (i.e. lorsque plusieurs Targets ont le même nom principal).
    docProject.generate_webpage_of_main_object() ;// génère la pages web des objets importants.
